...
def draw_detections(frame, detections, palette, labels, threshold, draw_landmarks=False):
    ...
    for detection in detections:
        if detection.score > threshold:
            ...
            if len(det_label) > 0:
                print(det_label)
                silence = AudioSegment.silent(duration=0.01*100)
                description = ', '.join(det_label)
                tts = gTTS(description, lang='en')
                tts.save('tts.mp3')
                tts = AudioSegment.from_mp3("tts.mp3")
                if first:
...
def main():
    ...
    #engine.runAndWait()
    #os.system("espeak" "Starting Detetction")
