...
import locale
#locale.setlocale(export LC_ALL=C)
...
#text recognition
ap.add_argument("-rec", "--rec", type=str, help="path to input text recognition model")
#
...
ap.add_argument("-p", "--padding", type=float, default=0.05, help='padding for ROI border')
...
#text recognition takes as input four points (x1,y1), (x2,y2), (x3,y3), (x4,y4) for each text bounding box.
...
input_info_rec = net_rec.input_info
#input name
input_blob_rec = next(iter(input_info_rec))
#input shape
input_shape = net_rec.input_info[input_blob_rec].input_data.shape
#output name
output_blob_rec = next(iter(net_rec.outputs))
#output shape
output_shape = net_rec.outputs[output_blob_rec].shape

#actually load model
exec_net_rec = ie.load_network(network=net_rec, device_name=args["device"])
...
while True:
    ...
    for ((startX, startY, endX, endY), angle) in zip(boxes, angles):
        ...
        #preprocess output from text detection and do things to make suitable input for recognition model
        
        input_img = preprocess_input(roi)
        
        #input shape
        
        input_dict = {input_blob_rec: input_img}
        
        
        #inference
        exec_net_rec.infer(input_dict)
        
        
        a = time.time()
        #res = exec_net_rec.infer(inputs=roi)
        res = exec_net_rec.infer(input_dict)
        b = time.time()
        #print('recognition time', b-a)
        ...
        #cv2.putText(orig, result, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (38,0,255), 1, cv2.LINE_AA)
        #cv2.putText(orig, result, (args["camera_width"]-170,15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (38,0,255), 1, cv2.LINE_AA)
...
"""
#THIS IS FOR THE TEXT RECOGNITION MODEL. NEED TO PREPROCESS THINGS
