...
def draw_detections(frame, detections, palette, labels, threshold):
    ...
    for detection in detections:
        if detection.score > threshold:
            ...
            detected_labels = open("/home/pi/build/open_model_zoo/demos/python_demos/object_detection_demo_py/det_labels.txt","wt")
            newlab = detected_labels.write(str(det_label))
            detected_labels.close()
            ...
            os.system('python3 /home/pi/build/open_model_zoo/demos/python_demos/object_detection_demo_py/text_to_speech.py')
            
            """
            
            if det_label and pygame.mixer.music.get_busy() == False:
                # and pygame.mixer.music.get_busy() == False
            name = str(det_label)+'.mp3'
                #pygame.mixer.init()
            if not os.path.isfile(name):
                tts = gTTS(text=str(det_label), lang='en')
                tts.save(name)
